...
void ofApp::setup(){
    ...
    room = ofMesh::box(5000, 3000, 5000, 1, 1, 1);
    room.setColorForIndices(0, room.getNumVertices(), ofFloatColor(1.0));
    for (int i = 0; i < room.getNumNormals(); i++){
        ofVec3f n = - room.getNormal(i);
        room.setNormal(i, n);
    }
...
void ofApp::setupDeferred(){
    ...
    shadowLightPass->setCam(75, 0.1, 4000);
...
void ofApp::updateDeferredParam(){
    ...
    shadowLightPass->setPosition(cos(ofGetElapsedTimef() * 0.2)*500, 1000.0, sin(ofGetElapsedTimef() * 0.2)*500);
    shadowLightPass->lookAt(ofVec3f(0.0));
